...
import { Plus, FileText, Download, Trash2, Eye, Calendar, Share2 } from "lucide-react";
...
const ReportsPage = () => {
  ...
  const [shareReport, setShareReport] = useState<{ id: string; title: string; shared_with: string[] } | null>(null);
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-3">
            {data.items.map((report) => (
              <ReportCard
                ...
                onShare={(report) => setShareReport({ id: report.id, title: report.title, shared_with: report.shared_with_user_ids || [] })}
                isExporting={exportMutation.isPending}
                canDelete={canCreateOrDelete}
                canShare={canCreateOrDelete} // Admin/Analyst can share
              />
            ))}
          </div>
        </div>
      ...
      {viewReport && (
        <ReportViewModal
          ...
          onDownload={() => handleExport(viewReport.id, viewReport.format)}
        />
      )}
      ...
      {shareReport && (
        <ShareReportModal
          ...
          currentSharedIds={shareReport.shared_with || []}
...
const ReportCard = ({
  ...
  onShare: (report: ReportResponse) => void;
  isExporting: boolean;
  canDelete: boolean;
  canShare: boolean;
}) => {
