...
import { Database, ShieldAlert, ShieldCheck, TrendingUp, BarChart3, CheckCircle2, XCircle, AlertCircle, AlertTriangle, Settings, RefreshCw, AlertOctagon, AlertCircle as AlertCircleIcon } from "lucide-react";

import { apiClient } from "@/lib/api";
import StatCard from "@/components/widgets/StatCard";
import TopMaliciousIPsWidget from "@/components/widgets/TopMaliciousIPsWidget";
    ...
    <div className="space-y-4">
      ...
      {(() => {
        ...
        if (isWidgetVisible('stat_total_queries', widgetVisibility)) {
          visibleStatCards.push(<StatCard key="stat_total_queries" title="Total Queries" value={stats.total_queries} subtitle={`+${stats.queries_today} today`} icon={TrendingUp} />);
        }
        ...
        if (isWidgetVisible('stat_unread_alerts', widgetVisibility)) {
          const unreadCount = data.recent_alerts?.filter(a => !a.is_read).length || 0;
          visibleStatCards.push(<StatCard key="stat_unread_alerts" title="Unread Alerts" value={unreadCount} subtitle="Requires attention" icon={AlertOctagon} />);
        }
      ...
      {(() => {
        ...
        if (isWidgetVisible('chart_query_trend', widgetVisibility)) {
          topRowWidgets.push(
            <div key="chart_query_trend" className="rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900/30 dark:bg-slate-900/30 light:bg-white p-4 overflow-hidden">
        ...
        if (isWidgetVisible('chart_risk_distribution', widgetVisibility)) {
          topRowWidgets.push(
            <div key="chart_risk_distribution" className="rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900/30 dark:bg-slate-900/30 light:bg-white p-4 overflow-hidden">
              <div className="flex items-center justify-between mb-4">
        ...
        if (isWidgetVisible('chart_ioc_type_distribution', widgetVisibility)) {
          topRowWidgets.push(
            <div key="chart_ioc_type_distribution" className="rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900/30 dark:bg-slate-900/30 light:bg-white p-4 overflow-hidden">
              <div className="flex items-center justify-between mb-4">
        ...
        if (data.cve_summary && isWidgetVisible('widget_cve_summary', widgetVisibility)) {
          topRowWidgets.push(
            <div key="widget_cve_summary" className="rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900/30 dark:bg-slate-900/30 light:bg-white p-4 overflow-hidden">
              <div className="flex items-center justify-between mb-4">
      ...
      {/* Bottom Row: API Status, Watchlist Summary */}
      {(() => {
        const bottomRowWidgets = [];
        if (isWidgetVisible('widget_api_status', widgetVisibility)) {
          bottomRowWidgets.push(
            <div key="widget_api_status" className="rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900/30 dark:bg-slate-900/30 light:bg-white p-4 overflow-hidden">
              <div className="flex items-center justify-between mb-4">
                  ...
                  data.api_status.map((api, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between">
                            ...
                            {api.limit ? (
                              <span className="text-xs text-slate-400 dark:text-slate-400 light:text-slate-600 whitespace-nowrap ml-2">
                                {api.usage_today} / {api.limit}
                              </span>
                            ) : (
                              api.usage_today !== undefined && (
                                <span className="text-xs text-slate-500 dark:text-slate-500 light:text-slate-500 whitespace-nowrap ml-2">
                          ...
                          {api.limit && (
                            <div className="mt-1.5 w-full bg-slate-800 dark:bg-slate-800 light:bg-slate-200 rounded-full h-1.5 overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all duration-500 ${((api.usage_today || 0) / api.limit) > 0.9 ? 'bg-red-500' :
                                  ((api.usage_today || 0) / api.limit) > 0.75 ? 'bg-yellow-500' : 'bg-brand-500'
                                  }`}
                                style={{ width: `${Math.min(100, ((api.usage_today || 0) / api.limit) * 100)}%` }}
                              />
                            </div>
                          )}

                          {api.remaining !== undefined && api.remaining !== null && (
                            <div className="flex justify-between items-center mt-1">
                              <span className="text-[10px] text-slate-500 dark:text-slate-500 light:text-slate-500">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs font-medium whitespace-nowrap ${api.status === "active"
        ...
        if (isWidgetVisible('widget_watchlist_summary', widgetVisibility)) {
          bottomRowWidgets.push(
            <div key="widget_watchlist_summary" className="rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900/30 dark:bg-slate-900/30 light:bg-white p-4 overflow-hidden">
              <div className="flex items-center justify-between mb-4">
      ...
      {(() => {
        ...
        if (isWidgetVisible('cve_chart_trend', widgetVisibility)) {
          visibleCveCharts.push(
            <div key="cve_chart_trend" className="rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900/30 dark:bg-slate-900/30 light:bg-white p-4">
              <div className="flex items-center justify-between mb-4">
        ...
        if (isWidgetVisible('cve_chart_cvss_distribution', widgetVisibility)) {
          visibleCveCharts.push(
            <div key="cve_chart_cvss_distribution" className="rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900/30 dark:bg-slate-900/30 light:bg-white p-4">
              <div className="flex items-center justify-between mb-4">
      ...
      {/* Security Widgets Section - Top Malicious IPs and Recent Alerts */}
      {(() => {
        const securityWidgets = [];
        if (isWidgetVisible('widget_top_malicious_ips', widgetVisibility)) {
          securityWidgets.push(
            <TopMaliciousIPsWidget key="widget_top_malicious_ips" data={data.top_malicious_ips || []} />
          );
        }
        if (isWidgetVisible('widget_recent_alerts', widgetVisibility)) {
          securityWidgets.push(
            <RecentAlertsWidget
              key="widget_recent_alerts"
              data={data.recent_alerts || []}
              onViewAll={() => navigate('/alerts')}
            />
          );
        }
        if (isWidgetVisible('widget_threat_map', widgetVisibility)) {
          securityWidgets.push(
            <ThreatMapWidget
              key="widget_threat_map"
              data={data.top_malicious_ips || []}
            />
          );
        }
        return securityWidgets.length > 0 ? (
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {securityWidgets.map((widget) => widget)}
          </section>
        ) : null;
      })()}
