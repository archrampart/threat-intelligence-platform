      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={async () => {
              try {
                const response = await apiClient.post('/api-keys/bulk-add-free');
                const data = response.data;
                alert(`✅ Success!\n\nAdded ${data.newly_added} free threat intelligence sources!\n\n${data.added_sources?.length > 0 ? `New sources: ${data.added_sources.join(', ')}` : 'All free sources were already added.'}`);
                queryClient.invalidateQueries({ queryKey: ["api-keys"] });
              } catch (error: any) {
                alert(`❌ Error: ${error.response?.data?.detail || error.message}`);
              }
            }}
            className="flex items-center gap-2 rounded-lg bg-green-600 dark:bg-green-600 light:bg-green-500 px-4 py-2 font-medium text-white transition hover:bg-green-700 dark:hover:bg-green-700 light:hover:bg-green-600"
          >
            <RefreshCw className="h-4 w-4" />
            Add All Free Sources
          </button>
        </div>
      </div>
            ...
            <div className="mb-2 flex items-center gap-2 text-sm text-slate-400 dark:text-slate-400 light:text-slate-600">
              ...
              <div className="flex gap-2 items-center">
                <button
                  ...
                  className={`rounded px-2 py-0.5 text-xs transition ${apiKey.update_mode === "manual"
                ...
                <button
                  ...
                  className={`rounded px-2 py-0.5 text-xs transition ${apiKey.update_mode === "auto"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/50 light:bg-black/30 p-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-800 dark:border-slate-800 light:border-slate-200 bg-slate-900 dark:bg-slate-900 light:bg-white p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {testMutation.data && (
            <div
              className={`rounded-lg border p-3 ${testMutation.data.success
