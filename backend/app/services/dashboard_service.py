...
from app.utils.geolocation import IPGeolocation
...
class DashboardService:
    ...
    def get_dashboard(self, user_id: str, is_admin: bool = False) -> DashboardResponse:
        ...
        try:
            ...
            top_malicious_ips = self._get_top_malicious_ips(user_id, is_admin)
        except Exception as e:
            logger.error(f"Error getting top malicious IPs: {e}", exc_info=True)
            top_malicious_ips = []

        try:
            # Get recent alerts
            recent_alerts = self._get_recent_alerts(user_id, is_admin)
        except Exception as e:
    ...
    def _get_api_status(self, user_id: str) -> List[APIStatus]:
        ...
        for api_key in api_keys:
            ...
            # Determine status
            # API key gerektirmeyen servisler (NONE) otomatik active
            if api_source.authentication_type.value == "none":
                status = "active"
            elif api_key.test_status.value == "invalid":
                status = "error"
            ...
            rate_limit_config = api_source.rate_limit_config or {}
            daily_limit = rate_limit_config.get("limit")
            remaining = None
            if daily_limit is not None and usage_today is not None:
    ...
    def _get_top_malicious_ips(self, user_id: str, is_admin: bool, limit: int = 10) -> List[TopMaliciousIP]:
        """Get top malicious IPs from IOC queries."""
        query_filter = IOCQuery.user_id == user_id if not is_admin else True
        ...
        results = (
            self.db.query(
                IOCQuery.ioc_value,
                func.max(IOCQuery.risk_score).label("max_risk"),
                func.count(IOCQuery.id).label("detection_count"),
                func.max(IOCQuery.query_date).label("last_seen"),
            )
            .filter(query_filter)
            .filter(IOCQuery.ioc_type == "ip")
            .filter(IOCQuery.risk_score.isnot(None))
            .filter(IOCQuery.risk_score >= 0.5)  # Medium and above
            .group_by(IOCQuery.ioc_value)
            .order_by(func.max(IOCQuery.risk_score).desc(), func.count(IOCQuery.id).desc())
            .limit(limit)
            .all()
        )
        ...
        for r in results:
            ...
            # Get sources for this IP
            sources = (
                self.db.query(ThreatIntelligenceData.source_api)
                .join(IOCQuery, ThreatIntelligenceData.ioc_query_id == IOCQuery.id)
                .filter(query_filter)
                .filter(IOCQuery.ioc_value == r.ioc_value)
                .filter(IOCQuery.ioc_type == "ip")
                .distinct()
                .all()
            )
            ...
            geo_data = IPGeolocation.get_location(r.ioc_value) or {}
            ...
            top_ips.append(
                TopMaliciousIP(
                    ...
                    sources=source_list[:5],  # Limit to 5 sources
    ...
    def _get_recent_alerts(self, user_id: str, is_admin: bool, limit: int = 10) -> List[RecentAlertItem]:
        """Get recent alerts for dashboard."""
        # Build query filter
        if is_admin:
            query = self.db.query(Alert).order_by(Alert.created_at.desc()).limit(limit)
        else:
            query = (
                self.db.query(Alert)
        ...
        for alert in alerts:
            ...
            if alert.watchlist_id:
                watchlist = (
                    ...
                    .filter(AssetWatchlist.id == alert.watchlist_id)
                    .first()
                )
                if watchlist:
            ...
            if alert.asset_id:
                asset = (
                    ...
                    .filter(AssetWatchlistItem.id == alert.asset_id)
                    .first()
                )
                if asset:
                    asset_value = asset.ioc_value
