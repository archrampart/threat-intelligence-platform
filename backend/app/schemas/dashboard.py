...
class APIStatus(BaseModel):
    ...
    remaining: Optional[int] = Field(None, description="Remaining requests today")
...
class TopMaliciousIP(BaseModel):
    """Top malicious IP information."""

    ip: str = Field(..., description="IP address")
    risk_score: float = Field(0.0, description="Risk score (0-100)")
    risk_level: str = Field("unknown", description="Risk level: low, medium, high, critical")
    detection_count: int = Field(0, description="Number of detections")
    last_seen: Optional[datetime] = Field(None, description="Last seen timestamp")
    sources: List[str] = Field(default_factory=list, description="Detection sources")
...
class RecentAlertItem(BaseModel):
    """Recent alert item for dashboard."""

    id: str = Field(..., description="Alert ID")
    title: str = Field(..., description="Alert title")
    severity: str = Field("medium", description="Alert severity: low, medium, high, critical")
    alert_type: str = Field(..., description="Alert type: watchlist, ioc_query, cve, system")
    is_read: bool = Field(False, description="Is alert read")
    created_at: datetime = Field(..., description="Alert creation time")
    watchlist_name: Optional[str] = Field(None, description="Related watchlist name")
    asset_value: Optional[str] = Field(None, description="Related asset value")
...
class DashboardResponse(BaseModel):
    ...
    top_malicious_ips: List[TopMaliciousIP] = Field(default_factory=list, description="Top 10 malicious IPs")
    recent_alerts: List[RecentAlertItem] = Field(default_factory=list, description="Recent alerts (last 10)")
